 ...
 * //TODO/`echo abc | offset.js 1 0` # doesn't work!?
 * ..
 */
...
const readFile = (_path, _a, _b) => {
	...
	const printResult = () => {
		...
		{
			if(_a !== null && _b === null && _a > offset)
			{
				console.error('Your offset exceeds limit (%d)!', offset);
				process.exit(4);
			}
		}
		...
		{
			if(_a > line && _b !== null && (stream && !(stream.closed || stream.destroyed)) && !(stream.closed || stream.destroyed))
			{
		...
		for(var i = 0; i < _chunk.length; ++i)
		{
			...
			if(_a !== null)
			{
				...
				else if(_a === line)
				{
					...
					{
						for(; offset < _chunk.length; ++offset, ++column)
						{
							if(_chunk[offset] === '\n' || _chunk[offset] === '\r')
							{
								break;
